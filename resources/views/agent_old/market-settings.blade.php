...
<body>
  <div class="main-wrapper Dashboard-bg admin-bg customResponsive">
                        ...
                        <table class="table table-striped table-bordered">
                          <thead>
                            <tr>
                              ...
                              <th class="align-C">Sport Name</th>
                              @endif
                              ...
                              @if(Request::segment(2)=='declare-market'  ||  Request::segment(2)=='match-lock' ||  Request::segment(2)=='market-rollback')
                         ...
                         @include('agent.market-settings-search')
                          </tbody>
                        </table>
                      </div>
                    </div>
    ...
    <script>
  function changestatus(status_name,status_value,id){
  $.ajax({
  url:'{{url()->current()}}',
  method:'GET',
  data:{status_name:status_name,status_value:status_value,id:id},
...
<script>
  function lockmatch(status_name,id,val){
    var checkbox = document.getElementById("inpLock"+id);    
    if (checkbox.checked) {
      var checkboxValue = 1;    
    }else{
      var checkboxValue = 0;      
    ...
    $.ajax({
      url: '{{url()->current()}}',
      method:'GET',
      data:{status_name:status_name,status_value:checkboxValue,id:id},
      success: function(data) {       
			console.log(data);
		}
    });
