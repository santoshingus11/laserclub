...
<section id="home-pages" class="home_pages">
        ...
        <div class="col-lg-7 col-md-12 col-sm-12">
          <div class="Home_pages_main_section">
            <div class="sports-wrapper block">
              <div class="events">
                   <h1 class="event_Cricket">Live Casino
                  <span class="starts-in-label d-block d-sm-none">Balance: {{$available_credit}}</span>
                </h1>
               <div class="row mb-5">

                            <div class="btn-group">
                                <button type="button" class="btn btn-dark btn-sm btn-block me-1 pricebtn" onclick="filterGames('evolution_lobby')">Evalution</button>
                                ...
                                <button type="button" class="btn btn-dark btn-sm btn-block me-1 pricebtn" onclick="filterGames('live_dealers')">Ezugi</button>
  ...
  <div class="container-fluid d-block d-sm-none">
    <div class="row" id="mobilegamesContainer">
        <!-- Your content here -->
    </div>
</div>
<script>
...
function filterGames(label) {
    const buttons = document.querySelectorAll('.pricebtn');
    buttons.forEach(button => {
        if (button.dataset.label === label) {
            button.classList.add('btn-primary'); // Add primary color class
            button.classList.remove('btn-dark'); // Remove dark color class
        } else {
            button.classList.remove('btn-primary'); // Remove primary color class
            button.classList.add('btn-dark'); // Add dark color class
        }
    });
...
function renderMobileGames(games){
     const gamesContainer = document.getElementById('mobilegamesContainer');
    gamesContainer.innerHTML = '';
    games.forEach(game => {
        const gameItem = document.createElement('div');
        gameItem.classList.add('col-md-6', 'col-lg-3', 'col-sm-6', 'col-xs-6');
